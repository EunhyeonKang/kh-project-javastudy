...
--  3) COUNT : 테이블에거 조건을 만족하는 행의 갯수 반환
SELECT COUNT(*) FROM EMPLOYEE WHERE SUBSTR(EMP_NO,8,1)=1;
--  4) MIN/MAX : 최소값/최대값
SELECT MIN(SALARY),MAX(SALARY) FROM EMPLOYEE;
SELECT SUM(SALARY) 급여합계, AVG(SALARY) 급여평균,COUNT(*) 직원수,MAX(SALARY) 최고급여,MIN(SALARY) 최소급여 FROM EMPLOYEE;


--1
SELECT EMP_NAME 직원명, EMAIL 이메일, LENGTH(EMAIL) 이메일길이 FROM EMPLOYEE;
--2
SELECT EMAIL,LPAD(EMAIL,INSTR(EMAIL,'@',1,1)-1,'#') 이메일아이디 FROM EMPLOYEE;
--3
SELECT EMP_NAME 직원명,SUBSTR(EMP_NO,1,2) 년생, NVL(BONUS,0) 보너스
FROM EMPLOYEE
WHERE SUBSTR(EMP_NO,1,2) BETWEEN 60 AND 69;
--4
SELECT COUNT(EMP_NAME) 인원 FROM EMPLOYEE WHERE SUBSTR(PHONE,1,3) != '010';
--5
...
--9
SELECT EMP_NAME, HIRE_DATE, FLOOR(SYSDATE-HIRE_DATE)-1 FROM EMPLOYEE;
--10
SELECT MAX(EXTRACT(YEAR FROM SYSDATE)-(SUBSTR(EMP_NO,1,2)+1900)+1) "최대 나이", MIN(EXTRACT(YEAR FROM SYSDATE)-(SUBSTR(EMP_NO,1,2)+1900)+1) "최소 나이" FROM EMPLOYEE;
--11
...
(EXTRACT(YEAR FROM SYSDATE)-(SUBSTR(EMP_NO,1,2)+1900)+1) 나이
FROM EMPLOYEE
...
FROM EMPLOYEE
JOIN DEPARTMENT ON (DEPT_CODE=DEPT_ID);
--INNER JOIN : 조인 조건식에 일치하는 데이터만 출력
SELECT EMP_NAME,DEPT_CODE,DEPT_TITLE
FROM EMPLOYEE
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID);
--OUTER JOIN : 조인 조건식에 일치하지 않는 데이터도 출력에 포함(LEFTMRIGHT,FULL)
SELECT * FROM EMPLOYEE;
--LEFT JOIN
SELECT EMP_NAME,DEPT_CODE,DEPT_TITLE
FROM EMPLOYEE
...
--CROSS JOIN : 카네이션 프로덕트, 조인되는 테이블의 각 행들이 모두 매핑
SELECT EMP_NAME, DEPT_CODE, DEPT_TITLE
FROM EMPLOYEE CROSS JOIN DEPARTMENT ORDER BY 1;

-- SELF JOIN : 자기자신과 JOIN 것
SELECT E1.EMP_NAME 사원이름,E1.MANAGER_ID, E2.EMP_NAME 관리자이름 
FROM EMPLOYEE E1
...
SELECT E1.EMP_NAME 사원이름,E1.MANAGER_ID 매니저아이디, M1.EMP_NAME 매니저이름, M1.EMP_ID 비교용매니저아이디
FROM EMPLOYEE E1
